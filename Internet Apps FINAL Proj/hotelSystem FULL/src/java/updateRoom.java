        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                ...
                if(RS.next()){
                    ...
                    RS = Stmt.executeQuery("SELECT * FROM room where hotelID ='"+hotelID+"' and roomID='"+rID+"' ");
                    if(RS.next()){
                    if(!airCInfo.equals("")){
                        String line2 = "update room set AirCond='"+airCInfo+"' where hotelID='"+hotelID+"' and roomID='"+rID+"'";
                        int Rows2 = Stmt.executeUpdate(line2);
                    }
                    if(!wifiInfo.equals("")){
                        String line2 = "update room set WIFI='"+wifiInfo+"' where hotelID='"+hotelID+"' and roomID='"+rID+"'";
                        int Rows2 = Stmt.executeUpdate(line2);
                    }
                    if(!TVInfo.equals("")){
                        String line2 = "update room set TV='"+TVInfo+"' where hotelID='"+hotelID+"' and roomID='"+rID+"'";
                        int Rows2 = Stmt.executeUpdate(line2);
                    }
                    if(!coffeInfo.equals("")){
                        String line2 = "update room set CoffeMachine='"+coffeInfo+"' where hotelID='"+hotelID+"' and roomID='"+rID+"'";
                        int Rows2 = Stmt.executeUpdate(line2);
                    }
                    
                    if(!rPhoto.equals("")){
                        String line2 = "update room set roomPic='"+rPhoto+"' where hotelID='"+hotelID+"' and roomID='"+rID+"'";
                        int Rows2 = Stmt.executeUpdate(line2);
                    }
                 }   
